...
int write_to_csv(double arr[], int nx, int ny, int nz, double D, double T, double dt, double lx, double ly, double lz, double c0, const char* filename);
...
void generate_bx_seq(double* C, double* bx, int nx, int ny, int nz);
void generate_by_seq(double* C, double* Cx, double* by, int* mask, int nx, int ny, int nz);
void generate_bz_seq(double* C, double* Cy, double* bz, int nx, int ny, int nz);
void update_C_seq(double* C, double* Cz, int* mask, int nx, int ny, int nz);

void solve_tridiag_thomas(double* l, double* d, double* u, double* x, double* b, int n);
...
int main(int argc, char* argv[])
{
    ...
    if (argc == 10) {
        output = new double[6];
        check_and_apply(output, argv[4], argv[5], argv[6], argv[7], argv[8], argv[9]);
        D = output[0];
        T = output[1];
        dt = output[2];
        lx = output[3];
        ...
        check_and_apply(output, argv[4], argv[5], argv[6], argv[7], argv[8], argv[9], argv[10], argv[11], argv[12], argv[13]);
        D = output[0];
        T = output[1];
        dt = output[2];
        lx = output[3];
    ...
    {
        host_constant[0] = sigma_x; 
        host_constant[1] = sigma_y;
        host_constant[2] = sigma_z;
        host_constant[3] = c0;

        double* bx = (double*)calloc(size, sizeof(double));
        double* by = (double*)calloc(size, sizeof(double));
        double* bz = (double*)calloc(size, sizeof(double));

        double* Cx = (double*)calloc(size, sizeof(double));
        double* Cy = (double*)calloc(size, sizeof(double));
        double* Cz = (double*)calloc(size, sizeof(double));
        
        std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
        printf("SIMULATING... USING CPU\n");
        // start simulation
        for (int i = 0; i < nt; i++)
        {
            // solve tridiagonal matrix for C'
            generate_bx_seq(C, bx, nx, ny, nz);
            solve_tridiag_thomas(Ax_a, Ax_b, Ax_c, Cx, bx, size);

            // solve tridiagonal matrix for C''
            generate_by_seq(C, Cx, by, mask, nx, ny, nz);
            solve_tridiag_thomas(Ay_a, Ay_b, Ay_c, Cy, by, size);
            
            // solve tridiagonal matrix for Ct+1
            generate_bz_seq(C, Cy, bz, nx, ny, nz);
            solve_tridiag_thomas(Az_a, Az_b, Az_c, Cz, bz, size);

            update_C_seq(C, Cz, mask, nx, ny, nz);
        ...
        fprintf(stderr, "ERROR, argument 3 wrong type %s\n", argv[3]);
        return 1;
    }
    ...
    fixEdges(C, nx, ny, nz);
    //print_arr_3d(C, n);
    int err = write_to_csv(C, nx, ny, nz, D, T, dt, lx, ly, lz, c0, output_name);
    if (err == 1) { return err; }
    return 0;
}
...
int write_to_csv(double arr[], int nx, int ny, int nz, double D, double T, double dt, double lx, double ly, double lz, double c0, const char* filename) {
    std::ofstream myfile;
    myfile.open(std::string(filename) + ".csv");
    ...
    for (int z = 0; z < nz; z++)
    {
        for (int y = 0; y < ny; y++)
        {
            for (int x = 0; x < nx; x++)
            {
                ...
                print_progress_bar(z * ny * nx + y * nx + x + 1, nx * ny * nz);
            }
        }
    }
    myfile << nx << "," << ny << "," << nz << "," << D << "," << T << "," << dt << "," << lx << "," << ly << "," << lz << "," << c0;
    myfile.close();
}
...
void generate_bx_seq(double* C, double* bx, int nx, int ny, int nz)
{
    for (int z = 0; z < nz - 2; z++)
    {
        for (int y = 0; y < ny - 2; y++)
        {
            for (int x = 0; x < nx - 2; x++)
            {
                double sigma_x = host_constant[0];
                double sigma_y = host_constant[1];
                double sigma_z = host_constant[2];
...
void solve_tridiag_thomas(double* l, double* d, double* u, double* x, double* b, int n)
{
    ...
    memcpy(dd, d, sizeof(double) * n);
    memcpy(uu, u, sizeof(double) * n);
